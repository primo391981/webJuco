...
@if ($pago->idTipoPago <> 2)
	<div class="form-group row">
		...
		<div class="col-md-9">
			<input type="checkbox"  name="gravado" id="gravado" class="checkbox" onChange="" {{ $pago->gravado==1 ? "checked" : "" }} autofocus>
		</div>
	</div>
	<div class="form-group row">
		<label for="porcentaje" class="control-label col-md-3">PORCENTAJE</label>
		<div class="col-md-6">
			<input type="number" name="porcentaje" id="porcentaje" class="form-control" value="{{ isset($pago) ? $pago->porcentaje : old('porcentaje') }}" min="0" max="100" {{ $pago->gravado==0 ? "disabled" : "" }} required >
